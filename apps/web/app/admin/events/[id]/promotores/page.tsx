...
type Agg = { codeId: string; code: string; label: string | null; entradas: number; recaudadoCents: number; descuentoCents: number };
...
export default async function PromotersPage({ params }: { params: { id: string } }) {
  ...
  for (const c of codes ?? []) {
    byCode.set(c.id, { codeId: c.id, code: c.code, label: c.label, entradas: 0, recaudadoCents: 0, descuentoCents: 0 });
  }
  type RedRow = {
    ...
    <>
      <div style={{ marginBottom: 14 }}>
        ...
        <p className="s-card__desc">
          {rows.length} código{rows.length === 1 ? '' : 's'} · {totalEntradas} entrada{totalEntradas === 1 ? '' : 's'} colocadas · {formatPEN(totalRecaudado)} recaudado
        </p>
      </div>
