  ...
  return (
    <section className="s-card">
      ...
      <p className="s-card__desc" style={{ marginBottom: 10 }}>
        Pega tus credenciales de MercadoPago para habilitar el pago con <strong>tarjeta</strong> en tu checkout. Las
        guardamos encriptadas; nunca las mostramos de vuelta. Si no las cargas, tu checkout sigue
      ...
      {!configured && !readOnly && (
        <details style={{ background: 'var(--paper-2)', borderRadius: 12, border: '1px solid var(--line)', marginBottom: 14, padding: '10px 14px' }}>
          ...
          <ol style={{ margin: '10px 0 0', paddingLeft: 18, fontSize: 13.5, lineHeight: 1.6, color: 'var(--ink-2)' }}>
            ...
            <li>Copia el <strong>Access Token</strong> y la <strong>Public Key</strong> (empiezan con <code>APP_USR-</code>) y pégalos aquí abajo.</li>
          </ol>
          <p style={{ margin: '8px 0 0', fontSize: 12.5, color: 'var(--ink-3)' }}>Validamos las credenciales con MercadoPago antes de guardar, así sabes al toque si están bien.</p>
        </details>
      )}
      ...
      <form action={action} className="s-stack" style={{ gap: 14 }}>
        ...
        <Field label="Access Token (APP_USR-… o TEST-…)" htmlFor="mp_access_token" error={err.mp_access_token}>
          <input id="mp_access_token" name="mp_access_token" type="password" autoComplete="off" className="s-input" disabled={readOnly}
            placeholder={hasAccessToken ? '•••••••••• (ya cargado — pega uno nuevo para reemplazar)' : 'APP_USR-0000000000000000-...'} />
        </Field>
        <Field label="Public Key" htmlFor="mp_public_key" error={err.mp_public_key}>
          <input id="mp_public_key" name="mp_public_key" type="password" autoComplete="off" className="s-input" disabled={readOnly}
            placeholder={hasPublicKey ? '•••••••••• (ya cargada — pega una nueva para reemplazar)' : 'APP_USR-xxxxxxxx-...'} />
        </Field>
