-- =============================================================
-- 0067 — Entrada PRIVADA gratis: UNA por persona (pedido de Paul, 2026-09-23)
-- =============================================================
-- "La entrada de Dybala que se pueda reclamar una vez nada más por persona."
-- Se aplica en reserve_order_stock, el embudo por el que pasan TODOS los
-- reclamos (startCheckout y claim_free_order, 0064) antes de que exista una
-- entrada, con advisory lock por (tipo, email) y (tipo, documento): dos toques
-- simultáneos de la misma persona → uno pasa, el otro 'private_claim_limit'.
-- Aplica a los tipos con fila en ticket_type_access reclamados a S/0
-- (order_items.unit_price_cents congelado). El resto de la función es la de la
-- 0061 sin cambios, salvo que el DOCUMENTO se compara en MAYÚSCULAS (un CE o
-- pasaporte con letras evadía los dos topes cambiando mayúsculas; hallazgo del
-- security-review, vale también para el tope por evento de la 0060).
...
as $$
  ...
  where o.event_id = p_event_id
    ...
    and (
      ...
      or (p_dni is not null and upper(nullif(btrim(o.buyer_dni), '')) = upper(p_dni))
    )
    and (
      o.status = 'paid'
      or (
...
begin
  ...
  -- ---------------------------------------------------------------
  -- GATE DE LA ENTRADA PRIVADA GRATIS (0067): una entrada privada (con link,
  -- 0066) que se reclama a S/0 vale UNA por persona. Misma idea y mismo tipo
  ...
  -- Las pagas no se topan (un promotor puede vender varias a un grupo).
  -- ---------------------------------------------------------------
  if v_metodo is distinct from 'courtesy' then
    for v_priv in
      ...
      where oi.order_id = p_order_id and oi.unit_price_cents = 0
        ...
        and (o.status = 'paid'
             or (o.status in ('pending_yape_review', 'pending_payment')
                 and exists (select 1 from public.stock_reservations sr
                             where sr.order_id = o.id and sr.expires_at > now())));
      if v_priv.qty > 1 or v_taken > 0 then
        raise exception 'private_claim_limit: type=% taken=% requested=%', v_priv.ttid, v_taken, v_priv.qty;
      end if;
    end loop;
  end if;
