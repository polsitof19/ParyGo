...
// en dos grupos de cuatro para leerlo y tipearlo sin perderse.
export async function sendCodigoAlta(a: { to: string; codigo: string; marca: string }): Promise<SendResult> {
  const grupos = a.codigo.length === 8 ? `${a.codigo.slice(0, 4)} ${a.codigo.slice(4)}` : a.codigo;
  const marca = a.marca.replace(/[<>&"']/g, '');
...
<tr><td style="font-family:${FONT};font-size:17px;line-height:1.5;color:#231C17;padding-bottom:20px;">Este es tu código para crear <strong>${marca}</strong> en ParyGo:</td></tr>
...
<tr><td style="font-family:${FONT};font-size:15px;line-height:1.5;color:rgba(35,28,23,0.70);padding-top:20px;">Escríbelo en la página donde te registraste. Vence en una hora.<br>Si no fuiste tú, ignora este correo: sin el código no se crea nada.</td></tr>
  ...
  const text = [
    `Tu código para crear ${marca} en ParyGo: ${grupos}`,
    '',
