...
try {
  ...
  check('en inglés sale en dólares', /US\$45/.test(html) && !/S\/150/.test(html));
  ...
  const boton = p.getByRole('button', { name: /^Pay US\$45$/ });
  check('el botón dice "Pay US$45"', await boton.count() === 1);
  await boton.click();
  ...
  const { data: marca } = await svc.from('brands').select('id, archived_at').eq('slug', slug).single();
  brandId = marca?.id;
  ...
  check('compra PayPal US$45 pendiente', compra?.provider === 'paypal' && compra.amount_cents === 4500 && compra.currency === 'USD' && compra.status === 'pending' && compra.created_by === null, JSON.stringify({ ...compra, provider_ref: compra?.provider_ref ? '…' : null }));
