      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        >
          {shown ? (
            ...
            <span className="s-muted-3" style={{ fontSize: 12, textAlign: 'center', padding: 8 }}>{t('Sin flyer', 'No flyer')}</span>
          )}
        </div>
        <div style={{ flex: 1, minWidth: 200 }}>
          {readOnly ? (
            <p className="s-card__desc">
              {currentUrl ? t('Flyer actual del evento (solo lectura).', "The event's current flyer (read-only).") : t('Este evento no tiene flyer cargado (solo lectura).', 'This event has no flyer uploaded (read-only).')}
            </p>
          ) : (
            <>
              <p className="s-card__desc" style={{ marginBottom: 10 }}>
                {t('Sube el flyer del evento (PNG, JPG o WEBP · vertical o cuadrado · máx 10 MB). Se ve grande en la portada del evento y en tu página de marca.', 'Upload the event flyer (PNG, JPG or WEBP · vertical or square · max 10 MB). It appears large on the event cover and on your brand page.')}
              </p>
              ...
              <div className="s-file">
                ...
                <label htmlFor="cover-file" className="s-btn s-btn--soft s-btn--sm s-file__btn">
                  {currentUrl ? t('Elegir otra imagen', 'Choose another image') : t('Elegir imagen', 'Choose image')}
                </label>
                <span className="s-file__name">{fileName ?? (currentUrl ? t('El actual', 'The current one') : t('Ninguno elegido', 'None chosen'))}</span>
              </div>
              <p className="s-hint">
              ...
              {avisoFlyer && (
                <p className="s-err" style={{ marginTop: 8 }}>
                  {t('Esto parece una captura de pantalla. ', 'This looks like a screenshot. ')}{avisoFlyer} {t('Puedes subirlo igual, pero si tienes el archivo original va a verse mucho mejor.', "You can upload it anyway, but if you have the original file it will look much better.")}
                </p>
              )}
...
function SubmitBtn({ hasFile, hasCurrent }: { hasFile: boolean; hasCurrent: boolean }) {
  ...
  return (
    <button type="submit" className="s-btn s-btn--primary s-btn--sm" disabled={pending}>
      <ImagePlus className="h-4 w-4" /> {pending ? t('Subiendo…', 'Uploading…') : hasCurrent ? t('Guardar flyer nuevo', 'Save new flyer') : t('Guardar flyer', 'Save flyer')}
    </button>
  );
}
